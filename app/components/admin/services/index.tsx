          ...
          <Form method="POST" action={`.${location.search}`}>
            ...
            <fieldset className="fieldset">
              <legend className="fieldset-legend">
                {translations.newCategoryTitle[lang]} (EST):
              </legend>
              <input
              ...
              {errors && errors.duplicatedFieldEE ? (
                <p className="label text-red-500">
                  {translations.categoryTitleTaken[lang]}
                </p>
              ) : null}
            </fieldset>
            ...
            <fieldset className="fieldset">
              <legend className="fieldset-legend">
                {translations.newCategoryTitle[lang]} (RUS):
              </legend>
              <input
...
export const CategoryListItem = ({
  ...
  return (
    <NavLink
      to={`/admin/services/${categoryId}${location.search}`}
      className={({ isActive }) =>
...
export const CategorySelectionList = ({
  ...
  return (
    <div className="w-full grid grid-cols-2 md:grid-cols-4 lg:grid-cols-4 xl:grid-cols-5 gap-x-5 gap-y-5 mt-5">
      {categories.map((item) => (
        <CategoryListItem
          key={item._id}
          categoryId={item._id}
